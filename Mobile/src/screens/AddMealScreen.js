...
function calcNutrition(food, grams) {
  ...
  return {
    carbsG:   food.carbsPer100g   != null ? +(food.carbsPer100g   * ratio).toFixed(1) : 0,
    calories: food.caloriesPer100g != null ? +(food.caloriesPer100g * ratio).toFixed(1) : 0,
    proteinG: food.proteinPer100g  != null ? +(food.proteinPer100g  * ratio).toFixed(1) : 0,
...
export default function AddMealScreen({ navigation, route }) {
  ...
  return (
    <ScrollView
      ...
      contentContainerStyle={[styles.scroll, { paddingTop: insets.top + theme.spacing.lg, paddingBottom: insets.bottom + theme.spacing.xxl }]}
    ...
    >
      ...
      <View style={styles.chipRow}>
        {MEAL_TYPES.map((opt) => (
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Text style={[styles.chipText, mealType === opt.value && styles.chipTextSelected]}>
              {opt.label}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      {/* ── Fotoğraf ile Tara ─────────────────────────────────────── */}
      <TouchableOpacity style={styles.cameraBtn} onPress={goCamera} activeOpacity={0.85}>
        ...
        <View style={styles.summaryCard}>
          ...
          <View style={styles.summaryRow}>
            <View style={styles.summaryTile}>
              <Text style={[styles.tileValue, { color: theme.colors.primary }]}>{totals.carbsG}<Text style={styles.tileUnit}> g</Text></Text>
              <Text style={styles.tileLabel}>Karbonhidrat</Text>
            </View>
            <View style={styles.tileDivider} />
            <View style={styles.summaryTile}>
              <Text style={styles.tileValue}>{totals.calories}<Text style={styles.tileUnit}> kcal</Text></Text>
              <Text style={styles.tileLabel}>Kalori</Text>
            </View>
            <View style={styles.tileDivider} />
            <View style={styles.summaryTile}>
              <Text style={styles.tileValue}>{totals.proteinG}<Text style={styles.tileUnit}> g</Text></Text>
              <Text style={styles.tileLabel}>Protein</Text>
            </View>
            <View style={styles.tileDivider} />
            <View style={styles.summaryTile}>
              <Text style={styles.tileValue}>{totals.fatG}<Text style={styles.tileUnit}> g</Text></Text>
      ...
      {items.length > 0 && (
        <View style={styles.card}>
          <Text style={styles.sectionLabel}>Eklenen Yemekler ({items.length})</Text>
              ...
              <View style={styles.itemRow}>
                <View style={styles.itemInfo}>
                  ...
                  {editingTempId === item.tempId ? (
                    ...
                    <Text style={styles.itemMeta}>
                      {item.amountGrams}g  ·  <Text style={{ color: theme.colors.primary, fontFamily: "Outfit_700Bold" }}>{item.carbsG}g karb</Text>
                ...
                <View style={styles.itemActions}>
                  <TouchableOpacity
                    ...
                    onPress={() => editingTempId === item.tempId ? confirmEdit(item) : startEdit(item)}
                  >
                    <Feather
                      name={editingTempId === item.tempId ? "check" : "edit-2"}
                      size={16}
                      color={editingTempId === item.tempId ? theme.colors.primary : theme.colors.textSecondary}
                    />
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.iconBtn} onPress={() => deleteItem(item.tempId)}>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.searchRow}>
          ...
          <TextInput
            ...
            value={searchQuery}
            onChangeText={(t) => { setSearchQuery(t); setSelectedFood(null); }}
        ...
        {(() => {
          const list = searchQuery.trim().length >= 2 ? searchResults : allFoods;
          if (isLoadingFoods && list.length === 0) {
            return <ActivityIndicator color={theme.colors.primary} style={{ marginTop: theme.spacing.md }} />;
          }
          ...
          return (
            <View style={styles.resultsList}>
              {list.map((food, idx) => (
                <TouchableOpacity
                  key={food.id}
                  style={[styles.resultItem, selectedFood?.id === food.id && styles.resultItemSelected, idx === list.length - 1 && { borderBottomWidth: 0 }]}
                  onPress={() => selectFood(food)}
                  activeOpacity={0.7}
                >
                  <Text style={styles.resultName}>{food.name}</Text>
                  {food.carbsPer100g != null && (
                    <Text style={styles.resultMeta}>{food.carbsPer100g}g karb / 100g  ·  {food.caloriesPer100g ?? "—"} kcal / 100g</Text>
                  )}
                </TouchableOpacity>
              ))}
            </View>
          ...
          <View style={styles.gramRow}>
            ...
            {gramInput && parseFloat(gramInput) > 0 && (
              <Text style={styles.gramPreview}>
                ≈ {calcNutrition(selectedFood, parseFloat(gramInput)).carbsG}g karb
              </Text>
            )}
...
const styles = StyleSheet.create({
  ...
  title: { ...theme.typography.title, color: theme.colors.textPrimary, marginBottom: theme.spacing.lg },

  chipRow: { flexDirection: "row", flexWrap: "wrap", gap: theme.spacing.sm, marginBottom: theme.spacing.md },
  chip: { borderWidth: 1, borderColor: theme.colors.border, borderRadius: 20, paddingHorizontal: theme.spacing.md, paddingVertical: theme.spacing.sm, backgroundColor: theme.colors.surface },
  ...
  cameraBtnText: { flex: 1, ...theme.typography.body, fontFamily: "Outfit_700Bold", color: theme.colors.primary },

  sectionLabel: { ...theme.typography.caption, fontFamily: "Outfit_700Bold", color: theme.colors.textPrimary, marginBottom: theme.spacing.md },
  ...
  tileValue: { fontSize: 18, fontFamily: "Outfit_700Bold", color: theme.colors.textPrimary },
  tileUnit: { fontSize: 12, fontFamily: "Outfit_400Regular", color: theme.colors.textSecondary },
  tileLabel: { ...theme.typography.caption, color: theme.colors.textSecondary, marginTop: 2, textAlign: "center", fontSize: 10 },
  ...
  itemInfo: { flex: 1 },
  itemName: { ...theme.typography.body, fontFamily: "Outfit_700Bold", color: theme.colors.textPrimary },
  itemMeta: { ...theme.typography.caption, color: theme.colors.textSecondary, marginTop: 2 },
  itemActions: { flexDirection: "row", gap: theme.spacing.sm },
  iconBtn: { padding: theme.spacing.sm },
  divider: { height: 1, backgroundColor: theme.colors.border },

  editGramRow: { flexDirection: "row", alignItems: "center", gap: 4, marginTop: 2 },
  editGramInput: {
    borderWidth: 1,
    borderColor: theme.colors.primary,
    borderRadius: 8,
  ...
  editGramUnit: { ...theme.typography.caption, color: theme.colors.textSecondary },

  searchRow: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderColor: theme.colors.border, borderRadius: 12, paddingHorizontal: theme.spacing.md, backgroundColor: theme.colors.background },
  searchIcon: { marginRight: theme.spacing.sm },
  searchInput: { flex: 1, height: 44, ...theme.typography.body, color: theme.colors.textPrimary },

  resultsList: { marginTop: theme.spacing.sm, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 12, backgroundColor: theme.colors.background, maxHeight: 260, overflow: "hidden" },
  resultItem: { paddingVertical: theme.spacing.md, paddingHorizontal: theme.spacing.md, borderBottomWidth: 1, borderBottomColor: theme.colors.border },
  ...
  resultMeta: { ...theme.typography.caption, color: theme.colors.textSecondary, marginTop: 2 },

  gramRow: { flexDirection: "row", alignItems: "center", gap: theme.spacing.sm, marginTop: theme.spacing.md },
  gramInputWrap: { flexDirection: "row", alignItems: "center", borderWidth: 1, borderColor: theme.colors.border, borderRadius: 12, paddingHorizontal: theme.spacing.md, backgroundColor: theme.colors.background },
  gramInput: { width: 72, height: 44, ...theme.typography.body, color: theme.colors.textPrimary },
  gramUnit: { ...theme.typography.caption, color: theme.colors.textSecondary },
  gramPreview: { ...theme.typography.caption, color: theme.colors.primary, fontFamily: "Outfit_700Bold", flex: 1 },
  addBtn: { backgroundColor: theme.colors.primary, borderRadius: 12, paddingHorizontal: theme.spacing.lg, height: 44, justifyContent: "center", alignItems: "center" },
  addBtnText: { ...theme.typography.caption, fontFamily: "Outfit_700Bold", color: "#fff" },

  saveBtn: { backgroundColor: theme.colors.primary, borderRadius: 14, minHeight: 52, alignItems: "center", justifyContent: "center", marginTop: theme.spacing.sm },
